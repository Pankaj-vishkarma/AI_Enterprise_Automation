...
require_ai_employee_use = require_permission(AI_EMPLOYEE_USE_PERMISSION)
require_ai_employee_manage = require_permission(AI_EMPLOYEE_MANAGE_PERMISSION)
...
def get_studio_config(current_user=Depends(require_ai_employee_use)):
    return {
...
def list_employees(current_user=Depends(require_ai_employee_use), db: Session = Depends(get_db)):
...
def create_employee(
    ...
    current_user=Depends(require_ai_employee_manage),
...
def get_employee(
    employee_id: int,
    current_user=Depends(require_ai_employee_use),
    db: Session = Depends(get_db),
):
    try:
